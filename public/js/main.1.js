...
function toggleInputOpt() {
    $('.toggle-input-overlay').toggle();
    if($('.chat-input').css('bottom') === '0px') {
        $('.chat-input').css('bottom', '-150px');
    } else {
        $('.chat-input').css('bottom', '0px');
    }
}

$('.toggle-input-opt').on('click', function() {
    toggleInputOpt();
    return;
});

$('.back-btn').on('click', function() {
    $(this).closest('.screen').css('left', '100%');
});
